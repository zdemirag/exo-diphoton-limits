...
bool makeLlrPlot = true;


// some global functions for plot styling
...
void CmsPrel( double x=0.13, double y=0.83){
  TLatex latex;
  latex.SetNDC();
...
void Cms( double x=0.13, double y=0.83){
  TLatex latex;
  latex.SetNDC();
  latex.SetTextSize(0.045);
...
void DrawTitle(std::string title = "", double x=0.10, double y=0.92){
  TLatex latex;
  latex.SetNDC();
  latex.SetTextSize(0.045);

  latex.SetTextAlign(5);
  //std::string sBuf = "asdf #font[62]{"+title+"}";
  latex.DrawLatex( x, y, title.c_str() );
...
void XLabel(std::string label = "", double x=0.95, double y=0.04){
  TLatex latex;
  latex.SetNDC();
  latex.SetTextSize(0.045);

  latex.SetTextAlign(31);
  latex.DrawLatex(x, y, label.c_str());
...
void YLabel(std::string label = "", double x=0.04, double y=0.95){
  TLatex latex;
  latex.SetNDC();
  latex.SetTextSize(0.045);
...
private:
  ...
  void PrintWsVars( void );

  double GammaUpper(double a, double x); // upper euler gamma func
  double BkgInt( double norm, double a, double b,
...
void TwoBody::PrintWsVars( void ){
  //
  // print all RooRealVars in the workspace
  //
...
double TwoBody::GetPoiUpperSimple(std::string channel, Double_t peak){
  ...
  double _width = ws->function(buf)->getVal();
  ...
  mData[channel]->fillHistogram(&h_count, *mpMc->GetObservables());
...
Double_t TwoBody::GetPoiUpper(std::string channel, Double_t peak){
  ...
  if (channel.find("dimuon")!=std::string::npos || 
      channel.find("dielectron")!=std::string::npos){
    _range = GetPoiUpperSimple(channel, peak);
  }
  else if (channel.find("dilepton")!=std::string::npos){
    _range = std::max(GetPoiUpperSimple("dimuon", peak), GetPoiUpperSimple("dielectron", peak));
  }
  else{
...
Int_t TwoBody::SetDielectronRatioParameters( Float_t peak ){
  ...
  //Float_t _low = std::max(0.0, 1.0-5.0*_sigma);
  //Float_t _high = 1.0+5.0*_sigma;
  //ws->var("beta_nsig_dielectron")->setRange(_low, _high);

  // FIXME: deal with nbkg prior
  //_sigma = ws->var("nbkg_kappa_dielectron")->getVal() - 1.0;
  //_low = std::max(0.0, 1.0-5.0*_sigma);
  //_high = 1.0+5.0*_sigma;
  //ws->var("beta_nbkg_dielectron")->setRange(_low, _high);

  //_sigma = ws->var("mass_kappa_dielectron")->getVal() - 1.0;
  //_low = std::max(0.0, 1.0-5.0*_sigma);
  //_high = 1.0+5.0*_sigma;
  //ws->var("beta_mass_dielectron")->setRange(_low, _high);
...
Int_t TwoBody::SetDiphotonParameters( Float_t peak, Float_t kmpl ){
  ...
  //ws->var("beta_nsig_diphoton")->setRange(_low, _high);

  //_sigma = ws->var("nbkg_kappa_diphoton")->getVal() - 1.0;
  //_low = std::max(0.0, 1.0-5.0*_sigma);
  //_high = 1.0+5.0*_sigma;

  //ws->var("beta_nbkg_diphoton")->setRange(_low, _high);
...
Int_t TwoBody::CreateDiphotonToyMc( Float_t peak, Float_t kmpl ){
  ...
  RooDataSet * _ds = ws->pdf("syst_nbkg_diphoton")->generate(*ws->var("beta_nbkg_diphoton"), 1);
...
Int_t TwoBody::CreateDielectronToyMc( void ){
  ...
  Double_t _beta = GetRandom("syst_nbkg_dielectron", "beta_nbkg_dielectron");
  Double_t _kappa = ws->var("nbkg_kappa_dielectron")->getVal();
  Double_t _nbkg_est = ws->var("nbkg_est_dielectron")->getVal();
  Double_t _ntoy = pow(_kappa,_beta) * _nbkg_est;

  Int_t _n = r.Poisson(_ntoy);

  // all nuisance parameters:
  //   beta_nsig_dielectron, 
  //   beta_nbkg_dielectron,
  //   beta_mass_dielectron,
  //   lumi_nuis
...
void TwoBody::makeMcmcPosteriorPlot( std::string filename ){
  ...
  if (mcInt && filename.size() > 0){
    ...
    _vNames.push_back("beta_nsig_dimuon");
    _vNames.push_back("beta_nbkg_dimuon");
    _vNames.push_back("beta_nsig_dielectron");
    _vNames.push_back("beta_nbkg_dielectron");
    _vNames.push_back("beta_mass_dielectron");

    plot_name = filename;

    for (std::vector<std::string>::const_iterator _name = _vNames.begin();
	 _name != _vNames.end();
	 ++_name){

      RooRealVar * _nuis = ws->var( _name->c_str() );
      if (_nuis){
	TCanvas c2("c2");
	plot.DrawChainScatter(*ws->var("ratio"), *_nuis);
	std::string _fname( _name->c_str() );
	_fname += "_vs_poi_"+plot_name;
	c2.SaveAs(_fname.c_str());
      }
    }
...
ModelConfig TwoBody::prepareDileptonRatioModel_old( std::string inputdir ){
  ...
  par.insert("ratio");
  par.insert("beta_nsig_dimuon");  
  par.insert("beta_nsig_dielectron");
  par.insert("beta_nbkg_dimuon"); 
  par.insert("beta_nbkg_dielectron");
  par.insert("beta_mass_dielectron");
  ...
  // nuisance
  RooArgSet sNuis( *(ws->var("beta_nsig_dimuon")),
		   *(ws->var("beta_nsig_dielectron")),
		   // *(ws->var("beta_nbkg_dimuon")),
		   // *(ws->var("beta_nbkg_dielectron")),
		   // *(ws->var("beta_mass_dimuon")),
		   *(ws->var("beta_mass_dielectron"))
		    );

  // global observables
  RooArgSet sGlobalObs( *(ws->var("beta_nsig_dimuon")),
		   *(ws->var("beta_nsig_dielectron")),
		   // *(ws->var("beta_nbkg_dimuon")),
		   // *(ws->var("beta_nbkg_dielectron")),
		   // *(ws->var("beta_mass_dimuon")),
...
ModelConfig TwoBody::prepareDileptonRatioModel( std::string inputdir,
  ...
  RooArgSet sNuis;
  if (do_dimuon){
    sNuis.add(*(ws->var("beta_nsig_dimuon")));
    sNuis.add(*(ws->var("beta_nbkg_dimuon")));
  }
  if (do_dielectron){
    sNuis.add(*(ws->var("beta_nsig_dielectron")));
    sNuis.add(*(ws->var("beta_nbkg_dielectron")));
    sNuis.add( *(ws->var("beta_mass_dielectron")) );
  }

  // define global observables
  RooArgSet sGlobalObs;
  if (do_dimuon){
    //sGlobalObs.add( *(ws->var("nom_beta_nsig_dimuon")) );
    sGlobalObs.add( *(ws->var("glob_nsig_dimuon")) );
    sGlobalObs.add( *(ws->var("glob_nbkg_dimuon")) );
  }
  if (do_dielectron){
    //sGlobalObs.add( *(ws->var("nom_beta_nsig_dielectron")) );
    sGlobalObs.add( *(ws->var("glob_nsig_dielectron")) );
    sGlobalObs.add( *(ws->var("glob_nbkg_dielectron")) );
    sGlobalObs.add( *(ws->var("glob_mass_dielectron")) );
  }
...
ModelConfig TwoBody::prepareThreeChannelModel( std::string inputdir ){
  ...
  par.insert("beta_nsig_dimuon");  
  par.insert("beta_nsig_dielectron");
  par.insert("beta_nsig_diphoton");
  par.insert("beta_nbkg_dimuon"); 
  par.insert("beta_nbkg_dielectron");
  par.insert("beta_nbkg_diphoton");
  par.insert("beta_mass_dielectron");
  ...
  // nuisance
  ws->var("beta_mass_dimuon")->setConstant(kTRUE);
  RooArgSet sNuis( *(ws->var("beta_nsig_dimuon")),
		   *(ws->var("beta_nsig_dielectron")),
		   *(ws->var("beta_nsig_diphoton")),
		   *(ws->var("beta_nbkg_dimuon")),
		   *(ws->var("beta_nbkg_dielectron")),
		   *(ws->var("beta_nbkg_diphoton")),
		   *(ws->var("beta_mass_dielectron")),
		   *(ws->var("lumi_nuis"))
		    );
...
void TwoBody::DimuonRatioLimit( Float_t peak,
  ...
  //ws->var("mass")->setRange(200.0, 3000.0);
...
//  }
  ...
  //if (plot_name.size() > 0){
  //  std::string _outfile = suffix + "_" + plot_name;
...
Double_t TwoBody::DileptonRatioSignificance( std::string mode,
  ...
  /* old (and misguided) way to define null parameter snapshot
  ...
  if (do_dimuon){
    ws->var("beta_nsig_dimuon")->setVal(0.0); // value for null hypothesis
    null_params.add( *ws->var("beta_nsig_dimuon") );

    ws->var("beta_nbkg_dimuon")->setVal(0.0); // value for null hypothesis
    null_params.add( *ws->var("beta_nbkg_dimuon") );
  }
  if (do_dielectron){
    ws->var("beta_nsig_dielectron")->setVal(0.0); // value for null hypothesis
    ws->var("beta_mass_dielectron")->setVal(0.0); // value for null hypothesis
    null_params.add( *ws->var("beta_nsig_dielectron") );
    null_params.add( *ws->var("beta_mass_dielectron") );

    ws->var("beta_nbkg_dielectron")->setVal(0.0); // value for null hypothesis
    null_params.add( *ws->var("beta_nbkg_dielectron") );
    //ws->var("beta_nbkg_dielectron")->setConstant(true);
   }
  std::string null_parameters_name = "NullParameters";
  ...
  */
  ...
  if (mode.find("data")!=std::string::npos){
    ...
    RooAbsReal * pNll = mc.GetPdf()->createNLL( *data );
    const RooArgSet * pGlobalObs = mc.GetGlobalObservables();
    RooArgSet poiAndGlobalObs("poiAndGlobalObs");
    poiAndGlobalObs.add( *pGlobalObs );
    const RooArgSet * pPoi = mc.GetParametersOfInterest();
    ...
    pPoiAndNuisance->add(*mc.GetNuisanceParameters());
    pPoiAndNuisance->add(*mc.GetParametersOfInterest());
    mc.SetSnapshot(*pPoiAndNuisance);

    // this snapshot is redundant
    // just to save time and not rename anything
    ws->defineSet( null_parameters_name.c_str(), *pPoiAndNuisance );
    ws->saveSnapshot(null_parameters_snapshot_name.c_str(), *ws->set(null_parameters_name.c_str()));
    ...
    std::string _outfile = channel+"_ratio_pllr_" + mode + suffix + ".ascii";
...
Double_t TwoBody::DileptonRatioSignificanceRobustness( std::string inputdir, // directory with workspace files
  ...
  if (option.find("low_range") != std::string::npos){
    ...
    for (Double_t _low = 200.0; _low <900.0; _low += 25.0){
      ...
      ws->var("mass")->setRange(_low, 3000.0);
...
std::pair<Double_t, Double_t> TwoBody::get_pllr_max( Double_t mass_low,
  ...
  std::string _legend = "[TwoBody::get_pllr_max]: ";
  ...
  for (Double_t _p = mass_low; _p < mass_high; _p += mass_step){
    ...
    std::cout << _legend << "DEBUG: pllr=" << _pllr << std::endl;
  ...
  ws->var("peak")->setRange(std::max(_max_mass-mass_step,mass_low),
...
double TwoBody::GammaUpper(double a, double x){
  //
  // inclomplete upper Gamma function
  //

  std::string legend = "[TwoBody::GammaUpper]: ";

  double result = TMath::Gamma(a)*( 1.0 - TMath::Gamma(a,x) );
...
double TwoBody::BkgInt( double norm, double a, double b,
			double x_min, 
			double x_max ){
  //
  // Integral of the background shape
  //
  
  std::string legend = "[TwoBody::BkgInt]: ";
  ...
  result = norm/a/pow(-a,b) * ( GammaUpper(b+1.0, -a*x_max) -
				GammaUpper(b+1.0, -a*x_min) );
...
double TwoBody::StudyBackground( void ){
  //
  // Do some checks about the background.
  // Potentially, change background normalization.
  //

  std::string legend = "[TwoBody::StudyBackground]: ";

  // integral of the background shape
  // int exp(a*x)*x^b = GammaUpper(b+1, -a*x)/(a*(-a)^b)

  double a_dimuon = ws->var("a_dimuon")->getVal();
  double b_dimuon = ws->var("b_dimuon")->getVal();

  double _int = BkgInt(1.0, a_dimuon, b_dimuon, 200, 3000);
...
Double_t significance( std::string mode,
  ...
  // adjust global settings first
  if (ntoys>1) makeLlrPlot = false;

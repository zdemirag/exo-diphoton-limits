...
for m_o in mass_ob:
    ...
    obs[j] = (float(s_observed[m_o][0]) + float(s_observed[m_o][1]))/2 #float(s_observed[m_o][len(s_observed[m_o])/2])
    print m_o,' ', obs[j]
    j = j+1
...
c4 = TCanvas("c4","Diphoton c = 005 Limits", 1000, 800)
...
latex2.DrawLatex(0.87, 0.95, "19.52 fb^{-1} at #sqrt{s} = 8 TeV")
